...
const addMoment = async (req, res) => {
    try {
        ...
        if (!result.rows.length) return res.status(statusCode.NOT_FOUND).send({ sucess: false, message: `Email not exists` })
        const idEmailQuery = idAndEmailExists(data.createdBy, data.userId)
        const checkIdAgainstEmail = await pgConnection(idEmailQuery)
        if (!checkIdAgainstEmail.rows.length) return res.status(statusCode.NOT_FOUND).send({ sucess: false, message: `Email-id and userId conflicts` })
        const addMomentQuery = add(data)
        const inserted = await pgConnection(addMomentQuery)
        if (!inserted.rowCount) return res.send({ success: false, message: `Error in inserting moment` })
        return res.send({ sucess: true, message: `Moment added successfully` });
    } catch (e) {
        return res.status(statusCode.BAD_REQUEST).send({ sucess: false, message: e });
    }
}
...
const getMoment = async (req, res) => {
    try {
        ...
        return res.send({ success: true, totalCount: Number(count.rows[0].count), getData: getData.rows })
    } catch (e) {
        return res.status(statusCode.BAD_REQUEST).send({ sucess: false, message: e });
    }
}
...
const updateMoment = async (req, res) => {
    try {
        ...
        if (!checkMomentId.rows.length) return res.status(statusCode.NOT_FOUND).send({ sucess: false, message: `Given moment id does not exists` })
        const updateMomentQuery = update(data, req.params.id)
        const updated = await pgConnection(updateMomentQuery)
        if (!updated.rowCount) return res.send({ success: false, message: `Error in updating moment` })
...
const deleteMoment = async (req, res) => {
    ...
    if (!deleted.rowCount) return res.send({ success: false, message: `Error in deleting moment` })
