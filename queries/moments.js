const add = (data) => {
  return `insert into moments (title, tags, image, created_by, user_id ) values ('${
    data.title
  }',
    '${data.tags}','${JSON.stringify(data.image)}','${data.createdBy}',${
    data.userId
  })`;
};
...
const update = (data, id) => {
  return `update moments set title = '${data.title}', tags = '${
    data.tags
  }', image = '${JSON.stringify(data.image)}', updated_by = '${
    data.updatedBy
  }', user_id = ${data.userId} where id = ${id} and active = 1`;
};
